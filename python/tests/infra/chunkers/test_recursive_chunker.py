...
class TestTextChunkerSingleChunk:
    def test_short_text_produces_single_chunk(self) -> None:
        ...
        chunker = TextChunker(max_tokens=500, overlap_tokens=100)
...
class TestTextChunkerParagraphSplits:
    def test_splits_at_paragraph_boundaries(self) -> None:
        para1 = "Word " * 15
        para2 = "Other " * 15
        text = para1.strip() + "\n\n" + para2.strip()
        pages = [PageContent(page_number=1, text=text)]
        chunker = TextChunker(max_tokens=20, overlap_tokens=0)
        chunks = chunker.chunk(pages)
        assert len(chunks) >= 2
    ...
    def test_each_chunk_respects_max_tokens(self) -> None:
        ...
        pages = [PageContent(page_number=1, text=text)]
        chunker = TextChunker(max_tokens=15, overlap_tokens=0)
        chunks = chunker.chunk(pages)
        for chunk in chunks:
...
class TestTextChunkerPageSpanning:
    ...
    def test_page_numbers_are_correct(self) -> None:
        ...
        chunker = TextChunker(max_tokens=500, overlap_tokens=0)
        chunks = chunker.chunk(pages)
        assert chunks[0].start_page == 1
        if len(chunks) == 1:
...
class TestTextChunkerSequentialIndices:
    def test_chunk_indices_are_sequential(self) -> None:
        text = "\n\n".join(["Paragraph " * 20 for _ in range(5)])
        pages = [PageContent(page_number=1, text=text)]
        chunker = TextChunker(max_tokens=30, overlap_tokens=0)
        chunks = chunker.chunk(pages)
        assert len(chunks) >= 2
...
class TestTextChunkerOverlap:
    def test_overlap_between_consecutive_chunks(self) -> None:
        ...
        pages = [PageContent(page_number=1, text=text)]
        chunker = TextChunker(max_tokens=30, overlap_tokens=10)
        chunks = chunker.chunk(pages)
        assert len(chunks) >= 2
...
class TestTextChunkerEmptyPages:
