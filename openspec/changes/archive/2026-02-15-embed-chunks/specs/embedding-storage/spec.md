...
### Requirement: EmbeddingRepository protocol defines vector storage contract
...
- `save_embeddings(provider_name: str, dimension: int, embeddings: list[EmbeddingVector]) → None` — store vectors
...
### Requirement: sqlite-vec storage adapter implements EmbeddingRepository
...
- **WHEN** `save_embeddings(provider_name, dimension, embeddings)` is called with a list of `EmbeddingVector` objects
...
### Requirement: Per-provider table naming convention
