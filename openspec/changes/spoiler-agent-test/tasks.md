...
- [ ] Write `TestSpoilerPreventionViaSearch::test_agent_answers_from_early_content_without_spoilers` — ask about early-book content, assert `judge_response` returns `True` with pre-decided expected behavior
- [ ] Write `TestSpoilerPreventionViaKnowledge::test_agent_refuses_to_reveal_ending` — ask "How does the book end?", assert `judge_response` returns `True` with pre-decided expected behavior describing what must NOT appear
