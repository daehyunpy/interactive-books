...
Vector similarity search using sqlite-vec KNN. Defines the `SearchResult` domain value object, extends `EmbeddingRepository` with a `search` method, and implements the sqlite-vec KNN adapter.

## Requirements

### VS-1: SearchResult value object represents a search hit
...
### VS-2: EmbeddingRepository search method performs KNN vector search
...
### VS-3: sqlite-vec adapter implements KNN search
